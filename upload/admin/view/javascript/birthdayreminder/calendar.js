...
Calendar.prototype.navigate = function (where, next) {
	...
	if (where == 'next') {
		...
		switch (this.options.view) {
		case 'month':
			if (this.options.position.start.getMonth() < 11)
		...
		case 'day':
			...
			else if (this.options.position.start.getDate() < 31)
				to.start.setDate(this.options.position.start.getDate() + 1);
			break;
		}
	} else if (where == 'prev') {
		switch (this.options.view) {
		case 'month':
			if (this.options.position.start.getMonth())
				to.start.setMonth(this.options.position.start.getMonth() - 1);
			break;
		case 'week':
			to.start.setDate(this.options.position.start.getDate() - 7);
			break;
		case 'day':
			if (this.options.position.start.getMonth() > 0)
				to.start.setDate(this.options.position.start.getDate() - 1);
			else if (this.options.position.start.getDate() > 1)
				to.start.setDate(this.options.position.start.getDate() - 1);
			break;
		}
	} else if (where == 'today') {
...
Calendar.prototype._update_month = function () {
	...
	$('a.event').mouseenter(function () {
		$('a[data-event-id="' + $(this).data('event-id') + '"]').closest('.cal-cell1').addClass('day-highlight');
	});
...
Calendar.prototype._update_month_year = function () {
	...
	var self = this;
	var activecell = 0;
	var downbox = $(document.createElement('div')).attr('id', 'cal-day-box').html('<i class="fa fa-sort-desc"></i>');
	...
	slider.slideUp('fast', function () {
		...
		slider.slideDown('fast', function () {
			$('.event-item').on('click', function (e) { 
				...
				$("#sendEmailModal .modal-body").load(target, function() {
