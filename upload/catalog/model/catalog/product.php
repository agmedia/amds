...
class ModelCatalogProduct extends Model {
    ...
    public function getLatestProductsB($limit) {
        $product_data = $this->cache->get('product.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . (int)$limit);

        if (!$product_data) {
            $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.status = '1'  AND p.quantity > '0'  AND p.date_available <= NOW()  AND pd.name  LIKE '%B21%'  ORDER BY p.sort_order DESC LIMIT " . (int)$limit);
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
                    ...
                    'sifra'                   => $product_option_value['sifra'],
	...
	public function getProductImages($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "'  ORDER BY sort_order ASC LIMIT 0,4");
    ...
    public function getProductRelated($product_id, $price) {
      ...
      //  $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . (int)$getCat->row['category_id'] . "' AND product_id != '" . (int)$product_id . "'  LIMIT 0,5");
            ...
            $query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2c.category_id = '" . (int)$getCat->row['category_id'] . "' AND p.product_id != '" . (int)$product_id . "' AND price > '".$price."' AND  p.status = '1' AND p.quantity > '0'  AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY RAND() LIMIT 0, 10");
    ...
    public function getProductRelatedNormal($product_id) {
        ...
        $getCat = $this->db->query("SELECT product_id, min(category_id) as category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id not in (SELECT distinct parent_id as category_id FROM oc_category)");
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id) WHERE (p2c.category_id = '" . (int)$getCat->row['category_id'] . "' AND (p2c.product_id != '" . (int)$product_id . "' AND p.status == '1'  LIMIT 0,5");
